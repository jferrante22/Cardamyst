...
export default function SegmentBreakdown({ data }) {
  ...
  const pieData = mainSegments.map((segment, index) => ({
    ...
    value: segment.coveredLives,
    totalLives: segment.totalLives,
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white px-3 py-2 rounded-lg shadow-lg border border-slate-200 text-xs">
          ...
          <p className="text-slate-500">Covered: {formatFullNumber(data.value)}</p>
          <p className="text-slate-500">Total Market: {formatFullNumber(data.totalLives)}</p>
          <p className="font-medium text-blue-600">{formatPercent(data.percent)} coverage</p>
        </div>
      );
    }
    return null;
  ...
  return (
    <div className="card h-full">
      <div className="card-header py-3 px-4">
        <h3 className="text-base font-semibold text-slate-800">Covered Lives Distribution</h3>
        <p className="text-xs text-slate-500">By market segment</p>
      </div>
